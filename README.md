...
本项目是一个 **AI 辅助的美股日报生成工具**，专为 [Ondo Global Markets](https://ondo.finance/) 社区设计。

**它能做什么？**
- 自动采集美股指数（标普500、纳指、道指）、个股、大宗商品实时行情
- 查询财报日历、市场开休市状态
- 验证股票是否在 Ondo 支持范围内
- 整合 X/Twitter 社区热议（通过 Grok）
- 生成结构化、可直接发布的 Markdown 日报

**工作方式**：你只需启动 AI 助手，说一句"生成今天的日报"，AI 会自动调用 6 个数据 Skills 采集信息，引导你补充社区热议数据，最终输出完整日报。
